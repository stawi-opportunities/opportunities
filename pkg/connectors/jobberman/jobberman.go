// Package jobberman implements a connector for the Jobberman job board.
package jobberman
...
// Connector fetches jobs from Jobberman.
type Connector struct {
	client *httpx.Client
}

// New creates a Jobberman Connector.
func New() *Connector {
	return &Connector{
...
// Type returns the SourceType for Jobberman.
func (c *Connector) Type() domain.SourceType { return domain.SourceJobberman }
...
func (it *iterator) Next(ctx context.Context) bool {
	...
	it.status = status
	if err != nil {
		log.Printf("jobberman: fetch page %d: %v", it.page, err)
		it.err = err
		return false
	}
	if status != 200 {
		log.Printf("jobberman: page %d status %d", it.page, status)
		it.err = fmt.Errorf("jobberman: status %d on page %d", status, it.page)
		return false
	}
	...
	for _, link := range links {
		...
		if err != nil {
			log.Printf("jobberman: detail %s: %v", link, err)
			continue
		}
