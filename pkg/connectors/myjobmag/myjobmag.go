// Package myjobmag implements a connector for the MyJobMag job board.
package myjobmag
...
var (
	jobLinkRe  = regexp.MustCompile(`href=["'](/job/[^"']+)["']`)
...
// Connector fetches jobs from MyJobMag.
type Connector struct {
	client *httpx.Client
}

// New creates a MyJobMag Connector.
func New() *Connector {
	return &Connector{
...
// Type returns the SourceType for MyJobMag.
func (c *Connector) Type() domain.SourceType { return domain.SourceMyJobMag }
...
func (it *iterator) Next(ctx context.Context) bool {
	...
	it.status = status
	if err != nil {
		log.Printf("myjobmag: fetch page %d: %v", it.page, err)
		it.err = err
		return false
	}
	if status != 200 {
		log.Printf("myjobmag: page %d status %d", it.page, status)
		it.err = fmt.Errorf("myjobmag: status %d on page %d", status, it.page)
		return false
	}
	...
	for _, link := range links {
		...
		if err != nil {
			log.Printf("myjobmag: detail %s: %v", link, err)
			continue
		}
