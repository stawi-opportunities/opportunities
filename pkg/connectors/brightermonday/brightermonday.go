// Package brightermonday implements a connector for the BrighterMonday job board.
...
// Connector fetches jobs from BrighterMonday.
type Connector struct {
	client *httpx.Client
}

// New creates a BrighterMonday Connector.
func New() *Connector {
	return &Connector{
...
// Type returns the SourceType for BrighterMonday.
func (c *Connector) Type() domain.SourceType { return domain.SourceBrighterMonday }
...
func (it *iterator) Next(ctx context.Context) bool {
	...
	it.status = status
	if err != nil {
		log.Printf("brightermonday: fetch page %d: %v", it.page, err)
		it.err = err
		return false
	}
	if status != 200 {
		log.Printf("brightermonday: page %d status %d", it.page, status)
		it.err = fmt.Errorf("brightermonday: status %d on page %d", status, it.page)
		return false
	}
	...
	for _, link := range links {
		...
		if err != nil {
			log.Printf("brightermonday: detail %s: %v", link, err)
			continue
		}
