// Package njorku implements a connector for the Njorku job board.
package njorku
...
var jobLinkRe = regexp.MustCompile(`href=["'](/job[s]?/[^"']+)["']`)

// Connector fetches jobs from Njorku.
type Connector struct {
	client *httpx.Client
}

// New creates a Njorku Connector.
func New() *Connector {
	return &Connector{
...
// Type returns the SourceType for Njorku.
func (c *Connector) Type() domain.SourceType { return domain.SourceNjorku }
...
func (it *iterator) Next(ctx context.Context) bool {
	...
	listURL := fmt.Sprintf("%s/search?page=%d", it.baseURL, it.page)
	raw, status, err := it.client.Get(ctx, listURL, nil)
	it.raw = raw
	it.status = status
	if err != nil {
		log.Printf("njorku: fetch page %d: %v", it.page, err)
		it.err = err
		return false
	}
	if status != 200 {
		log.Printf("njorku: page %d status %d", it.page, status)
		it.err = fmt.Errorf("njorku: status %d on page %d", status, it.page)
		return false
	}
