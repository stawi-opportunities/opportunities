#!/usr/bin/env bash
# Applies idx_opportunities_rt DDL to the running Manticore StatefulSet.
...
#
# Default DDL file: definitions/manticore/idx_opportunities_rt.sql
set -euo pipefail

REPO_ROOT=$(git -C "$(dirname "$(readlink -f "$0")")" rev-parse --show-toplevel)
DDL_FILE=${1:-"${REPO_ROOT}/definitions/manticore/idx_opportunities_rt.sql"}
...
kubectl -n opportunities exec -i svc/manticore -- \
    /usr/bin/mysql -h127.0.0.1 -P9306 -e "SHOW TABLES LIKE 'idx_opportunities_rt';" \
    | grep -q idx_opportunities_rt && echo "idx_opportunities_rt confirmed." || {
        echo "ERROR: idx_opportunities_rt not found after applying DDL"
        exit 1
    }
