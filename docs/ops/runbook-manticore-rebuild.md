...
**Symptom:** idx_jobs_rt corrupt, empty, or stale. Serving returns 503 or wrong results.

**Cause:** Manticore restart without persistent volume, or admin wiped the index.

**Recovery (60–120 min):**

1. Confirm R2 intact: aws s3 ls s3://stawi-jobs-log/canonicals_current/ | wc -l — expect ≥200.
2. curl -XPOST $MANTICORE_URL/sql?mode=raw -d "query=DROP TABLE IF EXISTS idx_jobs_rt"
3. Re-provision the schema: kubectl apply -f definitions/manticore/idx_jobs_rt.sql (or re-run Phase 2 migration).
...
7. Check row count every 10 min: curl -s $MANTICORE_URL/sql?mode=raw -d "query=SELECT COUNT(*) FROM idx_jobs_rt"
