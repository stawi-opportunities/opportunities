...
**Audience:** On-call operator deploying stawi.jobs to a fresh cluster (or after full DR recovery)
...
- [ ] Flux is running: `kubectl get helmreleases -n stawi-jobs`
- [ ] Managed infra ready:
  - [ ] Manticore StatefulSet running: `kubectl get pods -n stawi-jobs -l app=manticore`
  - [ ] Valkey reachable from within the cluster
  - [ ] R2 bucket `stawi-jobs-log` created with credentials at hand
...
4. `create-manticore-schema.sh` — applies `idx_jobs_rt` DDL to Manticore
...
# Apply the stawi-jobs namespace manifests
kubectl apply -k manifests/namespaces/stawi-jobs/

# Force Flux reconciliation
flux reconcile kustomization stawi-jobs --with-source
```

Wait for all six pods to reach `Ready`:
```bash
kubectl get pods -n stawi-jobs -w
```
...
Check current tags:
```bash
kubectl get imagepolicies -n stawi-jobs
```

To pin to a specific SHA:
```bash
# Edit the tag comment in the relevant HelmRelease, e.g.:
#   tag: v6.3.0 # {"$imagepolicy": "stawi-jobs:stawi-jobs-api:tag"}
```
...
Wait for `idx_jobs_rt` row count to cross the launch threshold (default 50k):
...
kubectl logs -f -l app=stawi-jobs-writer -n stawi-jobs | grep "parquet flushed"
...
1. Scale all six apps to 0:
   ...
   for app in api writer materializer crawler worker candidates; do
       kubectl scale deploy/stawi-jobs-${app} --replicas=0 -n stawi-jobs
   done
   ```
2. Revert HelmRelease image tags to the last known-good version (e.g., `v5.0.2`).
3. Apply the manifests: `kubectl apply -k manifests/namespaces/stawi-jobs/`
...
- [ ] `idx_jobs_rt` row count growing monotonically

---

## 5. Resource scaling

See `docs/ops/capacity-planning.md` and `manifests/namespaces/stawi-jobs/common/RESOURCE_SCALING.md`.
