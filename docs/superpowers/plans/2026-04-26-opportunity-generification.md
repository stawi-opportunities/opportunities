...
url_prefix: jobs
universal_required:
  [title, description, issuing_entity, apply_url, anchor_country]
kind_required: [employment_type]
kind_optional:
  [remote_type, seniority, skills, benefits, geo_restrictions, ats_platform]
categories:
  [
    Programming,
    Design,
...
```html
{{ $kind := .Params.kind | default "job" }} {{ with .Params.issuing_entity
}}<span class="text-sm text-gray-700">{{ . }}</span>{{ end }} {{ if eq $kind
"job" }} {{ with .Params.employment_type }}<span class="badge-type"
  >{{ . }}</span
>{{ end }} {{ if and .Params.amount_min (gt .Params.amount_min 0.0) }}
<span class="text-sm font-medium text-green-700">
  {{ .Params.currency }} {{ lang.FormatNumber 0 .Params.amount_min }}–{{
  lang.FormatNumber 0 .Params.amount_max }}
</span>
{{ end }} {{ else if eq $kind "scholarship" }} {{ with .Params.degree_level
}}<span class="badge-type">{{ . }}</span>{{ end }} {{ with
.Params.field_of_study }}<span class="badge-type">{{ . }}</span>{{ end }} {{
with .Params.deadline }}<span class="text-sm text-orange-700"
  >Apply by {{ dateFormat "Jan 2, 2006" . }}</span
>{{ end }} {{ else if eq $kind "tender" }} {{ with .Params.procurement_domain
}}<span class="badge-type">{{ . }}</span>{{ end }} {{ with .Params.deadline
}}<span class="text-sm text-orange-700"
  >Closes {{ dateFormat "Jan 2, 2006" . }}</span
>{{ end }} {{ else if eq $kind "deal" }} {{ with .Params.discount_percent
}}<span class="badge-type">{{ . }}% off</span>{{ end }} {{ with .Params.expiry
}}<span class="text-sm text-orange-700"
  >Expires {{ dateFormat "Jan 2, 2006" . }}</span
>{{ end }} {{ else if eq $kind "funding" }} {{ with .Params.focus_area }}<span
  class="badge-type"
  >{{ . }}</span
>{{ end }} {{ with .Params.deadline }}<span class="text-sm text-orange-700"
  >Apply by {{ dateFormat "Jan 2, 2006" . }}</span
>{{ end }} {{ end }}
```
      ...
      <label class="flex items-center gap-2 text-sm">
        <input
          ...
          onChange={(e) =>
            onChange({
              ...value,
              remote_ok: (e.currentTarget as HTMLInputElement).checked,
            })
          }
        />
        Open to remote
    ...
    <div class="space-y-2">
      ...
      <div class="flex gap-2">
        <input
          ...
          value={amountMin || ""}
          onInput={(e) =>
            onAmountMin(Number((e.currentTarget as HTMLInputElement).value))
          }
          class="input flex-1"
        />
        <input
          ...
          value={amountMax || ""}
          onInput={(e) =>
            onAmountMax(Number((e.currentTarget as HTMLInputElement).value))
          }
          class="input flex-1"
        />
        <input
    ...
    >
      ...
      <input
        ...
        onInput={(e) =>
          setP({
            ...p,
            target_roles: (e.currentTarget as HTMLInputElement).value
              .split(",")
              .map((s) => s.trim())
              .filter(Boolean),
          })
    ...
    >
      ...
      <input
        ...
        onInput={(e) =>
          setP({
            ...p,
            fields_of_study: (e.currentTarget as HTMLInputElement).value
              .split(",")
              .map((s) => s.trim())
              .filter(Boolean),
          })
      ...
      <input
        ...
        onInput={(e) =>
          setP({
            ...p,
            gpa_min: Number((e.currentTarget as HTMLInputElement).value),
          })
        }
        class="input"
        step="0.1"
...
export function Flow({
  ...
  const [p, setP] = useState<TenderPreferences>({
    company_name: "",
    ...
    >
      ...
      <input
        ...
        onInput={(e) =>
          setP({
            ...p,
            capabilities: (e.currentTarget as HTMLInputElement).value
              .split(",")
              .map((s) => s.trim())
              .filter(Boolean),
          })
...
export function Flow({ onSubmit }: { onSubmit: (p: DealPreferences) => void }) {
  const [p, setP] = useState<DealPreferences>({
    interest_categories: [],
    countries: [],
  });
  const CATEGORIES = [
    ...
    >
      ...
      <select
        ...
        onChange={(e) =>
          setP({
            ...p,
            organisation_type: (e.currentTarget as HTMLSelectElement).value,
          })
        }
      >
      ...
      <input
        ...
        onInput={(e) =>
          setP({
            ...p,
            focus_areas: (e.currentTarget as HTMLInputElement).value
              .split(",")
              .map((s) => s.trim())
              .filter(Boolean),
          })
...
const KINDS = [
  ...
  {
    kind: "scholarship",
    flow: "scholarship-onboarding-v1",
    label: "Scholarships",
  },
