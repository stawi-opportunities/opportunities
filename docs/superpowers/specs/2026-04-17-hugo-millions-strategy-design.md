...
- **The content tier in R2** — one `jobs/<slug>.json` snapshot per job, fronted by Cloudflare's CDN at a public subdomain `content.stawi.jobs`. Written directly by the publish handler. Reach millions of objects without ever involving Hugo.
...
│    2. PUT   content.stawi.jobs/jobs/<slug>.json (R2 public)    │
...
Single public R2 bucket `stawi-jobs-content`, custom domain `content.stawi.jobs`, CORS allowing `https://jobs.stawi.org` and local dev origins.
...
```json
{
  ...
  "company": {
    "name": "Acme Corp",
    "slug": "acme-corp",
    "logo_url": "https://content.stawi.jobs/companies/acme-corp/logo.png",
    "verified": true
  },
...
3. `fetch('https://content.stawi.jobs/jobs/' + slug + '.json')`.
...
2. Parallel: `GET content.stawi.jobs/categories/<slug>.json` (hero metadata) + `GET /api/categories/<slug>/jobs` (live list).
...
3. Optionally call CF "Purge Everything" once on the `content.stawi.jobs` zone.

`content.stawi.jobs` should be its own CF zone so a purge-everything doesn't wipe the app shell cache on `jobs.stawi.org`.
...
**Infra:**
- `content.stawi.jobs` as its own CF zone bound to the R2 bucket (separate from `jobs.stawi.org` so cache operations don't cross-affect).
