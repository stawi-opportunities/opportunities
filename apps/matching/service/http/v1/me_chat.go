...
// followUpReply acknowledges what we already captured, then asks for the
// single highest-priority missing required field.
func followUpReply(f onboardingChatFields, missing []string) string {
	if len(missing) == 0 {
		return "Thanks — I have everything I need to match opportunities. Choose a plan to start."
	}
	next := questionForMissing(missing[0])
	ack := acknowledgeKnown(f)
	if ack == "" {
		if len(missing) > 1 {
			return next + fmt.Sprintf(" After that I still need %d more detail(s).", len(missing)-1)
		}
		return next
	}
	if len(missing) > 1 {
		return ack + " " + next + fmt.Sprintf(" (%d more after that.)", len(missing)-1)
	}
	return ack + " " + next
}

func questionForMissing(key string) string {
	switch key {
	case "target_job_title":
...
// acknowledgeKnown builds a short summary of required fields already filled
// so multi-turn intake feels like the system is listening.
func acknowledgeKnown(f onboardingChatFields) string {
	var parts []string
	if t := strings.TrimSpace(f.TargetJobTitle); t != "" {
		parts = append(parts, t)
	}
	if v := normalizeExperience(f.ExperienceLevel); v != "" {
		parts = append(parts, v+" level")
	}
	if types := normalizeJobTypes(f.JobTypes); len(types) > 0 {
	...
	if countries := normalizeCountryList(f.PreferredCountries); len(countries) > 0 {
		parts = append(parts, "opportunities from "+strings.Join(countries, ", "))
	} else if c := normalizeCountry(f.Country); c != "" {
		parts = append(parts, "based in "+c)
	}
	if hasCapabilities(f) {
