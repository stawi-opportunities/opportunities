...
DO $$
BEGIN
    IF to_regclass('crawl_jobs') IS NULL THEN
        RETURN;
    END IF;
    IF EXISTS (SELECT 1 FROM timescaledb_information.hypertables
               WHERE hypertable_name = 'crawl_jobs') THEN
    ...
    -- it defensively in case an environment still carries the legacy (id) PK.
    IF NOT EXISTS (
...
DO $$
BEGIN
    IF to_regclass('raw_payloads') IS NULL THEN
        RETURN;
    END IF;
    IF EXISTS (SELECT 1 FROM timescaledb_information.hypertables
               WHERE hypertable_name = 'raw_payloads') THEN
        RETURN;
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint c
