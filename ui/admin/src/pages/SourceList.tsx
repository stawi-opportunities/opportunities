...
import { listSources, rescoreSource, type SourceListItem } from '@/api/admin-client';
import { Button, Card, ErrorBlock, LoadingSkeleton, StatusBadge, useToast } from '@/components/ui';

const PAGE_SIZE = 25;

function healthBadge(score: number) {
  if (score >= 0.7) return <StatusBadge variant="success" label={score.toFixed(2)} size="sm" />;
  if (score >= 0.4) return <StatusBadge variant="warning" label={score.toFixed(2)} size="sm" />;
  return <StatusBadge variant="error" label={score.toFixed(2)} size="sm" />;
}

function statusBadge(status: string) {
  switch (status) {
    case 'active': return <StatusBadge variant="success" label="active" size="sm" />;
    case 'paused': return <StatusBadge variant="warning" label="paused" size="sm" />;
    case 'error': return <StatusBadge variant="error" label="error" size="sm" />;
    default: return <StatusBadge variant="neutral" label={status} size="sm" />;
  }
}
...
export function SourceList() {
  ...
  if (error) return <ErrorBlock message="Failed to load sources" detail={String(error)} />;
              ...
              <tbody>
                {filtered.map((s: SourceListItem) => (
                  <tr key={s.id}>
                    ...
                    <td>{s.country}</td>
                    <td>{typeof s.health_score === 'number' ? healthBadge(s.health_score) : '—'}</td>
                    <td>
                    ...
                    <td>
                      <Button
                        size="sm"
                        variant="ghost"
                        loading={rescore.isPending && rescore.variables === s.id}
