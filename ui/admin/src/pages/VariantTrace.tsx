...
import { getVariantTrace, reparseRawPayload } from "@/api/admin-client";
...
export function VariantTrace() {
  ...
  const reparse = useMutation({
    ...
    onSuccess: (res) => {
      toast(`Queued ${res.queued} raw_payload(s) for re-extraction.`, {
        type: "success",
      });
    },
    onError: (e: Error) => {
  ...
  if (error)
    return (
      <ErrorBlock message="Failed to load variant" detail={String(error)} />
    );
  if (!data) return null;
          ...
          <table>
            <tbody>
              ...
              <tr>
                ...
                <td>
                  {new Date(data.raw_payload.fetched_at).toLocaleString()}
                </td>
              </tr>
            </tbody>
          </table>
