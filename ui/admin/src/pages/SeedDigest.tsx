...
export function SeedDigest() {
  ...
  const reparse = useMutation({
    ...
    onSuccess: (res) => {
      toast(`Queued ${res.queued} raw_payload(s) for re-extraction (window: ${res.window_seconds ?? '?'}s).`, {
        type: 'success',
      });
    },
    onError: (e: Error) => {
  ...
  if (error) return <ErrorBlock message="Failed to load digest" detail={String(error)} />;
      ...
      <div style={{ marginBottom: '1.25rem' }}>
        <h1 style={{ margin: 0 }}>
          ...
          <span style={{ fontWeight: 400, fontSize: '0.9rem', color: 'var(--c-text-secondary)' }}>
            for <Link to={`/sources/${encodeURIComponent(id)}`}>{id}</Link> on {date}
          </span>
        </h1>
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginTop: '0.5rem' }}>
          ...
          <Button variant="outline" size="sm" loading={reparse.isPending} onClick={() => reparse.mutate()}>
            Reparse last 24h
          </Button>
        </div>
      </div>
      ...
      {data && (
        <>
          <Card title="Counts" style={{ marginBottom: '1rem' }}>
            ...
            <p style={{ margin: '0.5rem 0 0', fontSize: '0.8rem', color: 'var(--c-text-secondary)' }}>
