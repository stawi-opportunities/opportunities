      ...
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
        <div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {OPPORTUNITY_TYPES.map(({ href, emoji, label, description, color, badge }) => (
              <a key={href} href={href} className={`group flex flex-col rounded-xl border p-5 transition-all duration-150 ${color}`}>
                <div className="flex items-center gap-3">
                  <span className={`flex h-10 w-10 items-center justify-center rounded-lg text-xl ${badge}`}>{emoji}</span>
                  <span className="text-base font-semibold text-gray-900 group-hover:text-navy-900">{label}</span>
                  <svg className="ml-auto h-4 w-4 text-gray-400 transition-transform group-hover:translate-x-0.5 group-hover:text-navy-600" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
        ...
        {!q.isLoading && cats.length > 0 && (
          <div className="mt-14">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
              {cats.map((c) => (
                <a key={c.key} href={`/categories/${encodeURIComponent(c.key)}/`} className="group flex flex-col justify-between rounded-lg border border-gray-200 bg-white p-4 transition-all hover:border-navy-300 hover:shadow-sm">
                  <div className="font-medium capitalize text-gray-900 group-hover:text-navy-900">{c.key || "Uncategorised"}</div>
                  <div className="mt-1 text-xs text-gray-400">{c.count.toLocaleString()} {c.count === 1 ? "opportunity" : "opportunities"}</div>
                </a>
              ))}
            </div>
          </div>
