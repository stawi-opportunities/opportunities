...
function BrowseDropdown() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        Browse
        <svg className={`h-4 w-4 transition-transform duration-150 ${open ? "rotate-180" : ""}`}
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 w-52 rounded-xl border border-gray-100 bg-white shadow-lg ring-1 ring-black/5">
          <div className="p-1.5">
            {browseItems.map(({ href, emoji, label, sub }) => (
              <a key={href} href={href} onClick={() => setOpen(false)}
                ...
                <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-md bg-gray-100 text-base">{emoji}</span>
                <div>
          ...
          <div className="border-t border-gray-100 px-3 py-2">
            <a href="/search/" className="flex items-center gap-1.5 text-xs font-medium text-navy-700 hover:text-navy-900">
