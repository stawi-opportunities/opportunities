...
const browseItems: Array<{
  ...
  { href: '/jobs/', icon: 'briefcase', label: 'Jobs', sub: 'Full-time, remote & more' },
  {
    href: '/scholarships/',
    icon: 'graduation',
    label: 'Scholarships',
  ...
  },
  {
    ...
    icon: 'clipboard',
    label: 'Tenders',
    sub: 'RFPs & procurement',
    comingSoon: true,
  },
  { href: '/deals/', icon: 'tag', label: 'Deals', sub: 'Curated discounts', comingSoon: true },
  {
    href: '/funding/',
    icon: 'money',
    label: 'Funding',
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 w-52 rounded-xl border border-gray-100 bg-white shadow-lg ring-1 ring-black/5 dark:border-navy-700 dark:bg-navy-900">
          <div className="p-1.5">
            {browseItems.map(({ href, icon, label, sub, comingSoon }) =>
              comingSoon ? (
                <button
                  key={href}
                  type="button"
                  ...
                  className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-navy-900 dark:text-gray-300 dark:hover:bg-navy-800 dark:hover:text-white text-left"
                >
...
function MobileMenu({ open, isAuth }: { open: boolean; isAuth: boolean }) {
  ...
  return (
    <div className="border-t border-gray-100 bg-white px-4 pb-4 pt-2 md:hidden animate-slide-down dark:border-navy-700 dark:bg-navy-900">
      ...
      {browseItems.map(({ href, icon, label, comingSoon }) =>
        comingSoon ? (
          <button
            key={href}
            type="button"
            onClick={() => toast(t('common.comingSoon'), 'info')}
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-navy-800 text-left"
          >
            <Icon
              name={icon}
              size={16}
