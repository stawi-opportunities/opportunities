...
export default function OpportunityDetail() {
  ...
  useEffect(() => {
    ...
    const engagedAt = setTimeout(() => {
      ...
      const scrollPct = doc
        ? Math.min(100, Math.round(((window.scrollY + window.innerHeight) / (doc.scrollHeight || 1)) * 100))
        : 0;
      trackJobViewEngaged({
        canonical_job_id: snap.id,
        slug: snap.slug,
  ...
  useEffect(() => {
    ...
    if (!el) return;
    if (!q.data || q.data.kind !== 'job') { el.textContent = ''; return; }
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {expired && (
        <div className="mt-4 rounded-md border border-amber-300 bg-amber-50 px-4 py-2 text-sm text-amber-900" role="status">
          {expiredMessage(snap.kind, t)}
        </div>
      )}
      {showTranslatedNotice && (
        <div className="mt-4 rounded-md border border-sky-200 bg-sky-50 px-4 py-2 text-sm text-sky-900" role="status">
      ...
      <header className="mt-4 flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm text-gray-700"><span className="font-medium">{snap.issuing_entity}</span></p>
          <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-gray-600">
            ...
            {snap.remote && <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs">{t('job.remote')}</span>}
            {snap.posted_at && <span className="text-gray-400">{t('job.postedOn')} {timeAgo(snap.posted_at)}</span>}
            {snap.deadline && !expired && <span className="text-orange-700">{deadlineLabel(snap.kind, t)} {new Date(snap.deadline).toLocaleDateString()}</span>}
          </div>
...
function ApplyLink({ snap, mountedAtRef, t, large = false }: { snap: OpportunitySnapshot; mountedAtRef: { current: number }; t: (k: StringKey, fallback?: string) => string; large?: boolean }) {
  const className = large ? 'btn-primary px-8 py-3 text-base' : 'btn-primary';
  return (
    <a href={snap.apply_url} target="_blank" rel="noopener noreferrer"
      onClick={() => {
        trackApplyClick({ canonical_job_id: snap.id, slug: snap.slug, company: snap.issuing_entity, apply_url: snap.apply_url ?? '', dwell_ms: Math.round((typeof performance !== 'undefined' ? performance.now() : Date.now()) - mountedAtRef.current) });
        void pingApply(snap.slug);
      }}
      className={className}>
...
function Breadcrumbs({ prefix, category, t }: { prefix: string; category?: string; t: (k: StringKey, fallback?: string) => string }) {
  return (
    <nav aria-label="Breadcrumb" className="text-sm text-gray-500">
      ...
      {category && (<><span className="mx-1.5">/</span><a href={`/categories/${encodeURIComponent(category)}/`} className="hover:text-gray-700">{categoryLabel(category)}</a></>)}
    </nav>
  );
}

function IssuingEntityAvatar({ snap }: { snap: OpportunitySnapshot }) {
  const logo = typeof snap.attributes?.logo_url === 'string' ? (snap.attributes.logo_url as string) : undefined;
  if (logo) {
    return <img src={logo} alt={`${snap.issuing_entity} logo`} className="h-14 w-14 shrink-0 rounded-lg border border-gray-200 object-contain bg-white" loading="lazy" />;
  }
  const initial = (snap.issuing_entity || '?').trim().slice(0, 1).toUpperCase();
  return <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded bg-navy-100 text-xl font-semibold text-navy-900" aria-hidden="true">{initial}</div>;
}

function ShareButton({ title, subtitle, t }: { title: string; subtitle: string; t: (k: StringKey, fallback?: string) => string }) {
  const canShare = typeof navigator !== 'undefined' && 'share' in navigator;
  async function onClick() {
    const url = window.location.href;
    if (canShare) { try { await navigator.share({ title, text: `${title} — ${subtitle}`, url }); return; } catch { /* fall through */ } }
    try { await navigator.clipboard.writeText(url); } catch { /* noop */ }
  }
  return (
    <button type="button" onClick={onClick} className="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
...
function buildJobPostingLd(snap: OpportunitySnapshot): Record<string, unknown> {
  const ld: Record<string, unknown> = {
    ...
    title: snap.title, description: snap.description_html ?? snap.description,
    datePosted: snap.posted_at, validThrough: snap.expires_at ?? snap.deadline,
    employmentType: typeof snap.attributes?.employment_type === 'string' ? snap.attributes.employment_type : undefined,
    hiringOrganization: { '@type': 'Organization', name: snap.issuing_entity, logo: typeof snap.attributes?.logo_url === 'string' ? snap.attributes.logo_url : undefined },
  };
  if (snap.anchor_location) {
    ld.jobLocation = { '@type': 'Place', address: { '@type': 'PostalAddress', addressLocality: snap.anchor_location.city, addressRegion: snap.anchor_location.region, addressCountry: snap.anchor_location.country } };
  }
  if (snap.amount_min || snap.amount_max) {
    const period = typeof snap.attributes?.salary_period === 'string' ? (snap.attributes.salary_period as string) : 'year';
    ld.baseSalary = { '@type': 'MonetaryAmount', currency: snap.currency || 'USD', value: { '@type': 'QuantitativeValue', minValue: snap.amount_min, maxValue: snap.amount_max, unitText: period.toUpperCase() } };
  }
  return ld;
}
...
function NotFound({ kind, t }: { kind: OpportunityKind | undefined; t: (k: StringKey, fallback?: string) => string }) {
  ...
  return (
    <div className="mx-auto max-w-md py-16 text-center">
      ...
      <a href={browseHref} className="btn-primary mt-6">{t('cta.browseAll')}</a>
    </div>
  );
}
...
function LoadError({ onRetry, t }: { onRetry: () => void; t: (k: StringKey, fallback?: string) => string }) {
  return (
    <div className="mx-auto max-w-md py-16 text-center">
      ...
      <button type="button" onClick={onRetry} className="btn-primary mt-6">{t('cta.tryAgain')}</button>
    </div>
  );
}