        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
            {CHIPS.map((chip) => {
              ...
              return (
                <button key={`${chip.key}-${chip.value}`} type="button" onClick={() => toggle(chip)} aria-pressed={on}
                  className={["inline-flex items-center rounded-full border px-4 py-1.5 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-navy-500",
                    on ? "border-navy-900 bg-navy-900 text-white" : "border-gray-300 bg-white text-gray-700 hover:border-navy-700 hover:text-navy-900",
                  ].join(" ")}
                >
                  {chip.label}
                </button>
            ...
            {hasFilters && (
              <button type="button" onClick={() => setActive({})} className="ml-1 text-sm text-gray-500 hover:text-gray-900 underline">
                Clear
              </button>
            )}
          </div>
          <div className="mt-6">
            <Cascade filters={filters} preferredCountries={preferredCountries} preferredLanguages={preferredLanguages} tierLimit={25} />
          </div>
        </div>
        <aside className="hidden w-64 shrink-0 lg:block">
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-4">
              {["remote_type", "employment_type", "seniority"].map((key) => (
                <div key={key}>
                  <label className="block text-xs font-medium uppercase tracking-wide text-gray-500">
                    {key === "remote_type" ? "Remote" : key === "employment_type" ? "Employment" : "Level"}
                  </label>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {CHIPS.filter((c) => c.key === key).map((c) => {
                      const on = active[c.key as FilterChip["key"]] === c.value;
                      return (
                        <button key={c.value} type="button" onClick={() => toggle(c)} aria-pressed={on}
              ...
              {hasFilters && (
                <button type="button" onClick={() => setActive({})} className="w-full rounded-md border border-gray-200 py-1.5 text-xs font-medium text-gray-500 hover:border-gray-400 hover:text-gray-900 transition-colors">
                  Clear all filters
                </button>
              )}
            </div>
